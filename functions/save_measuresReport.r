...
### save contacting category infromation - total in category, total numbe rof contacts caused by each
### category and number of contacts per dog in each category Emily Oct 2017
## need to change back when I27 test done 
if (kernel_type == "category" | kernel_type == "category" ) {
  nb_explorer <- length(which(allDogs$category == "Explorer"))
  nb_roamer <- length(which(allDogs$category == "Roamer"))
  nb_stayhome <- length(which(allDogs$category == "Stayhome"))
  
  nb_contacting_explorer <- length(which(allDogs$contacting_category == "Explorer"))
  nb_contacting_roamer <- length(which(allDogs$contacting_category == "Roamer"))
  nb_contacting_stayhome <- length(which(allDogs$contacting_category == "Stayhome"))
  
  contact_per_explorer <- nb_contacting_explorer/nb_explorer
  contact_per_roamer <- nb_contacting_roamer/nb_roamer
  contact_per_stayhome <- nb_contacting_stayhome/nb_stayhome
} else {
  nb_explorer <- NA
  nb_roamer <- NA
  nb_stayhome<- NA
  nb_contacting_explorer<- NA
  nb_contacting_roamer<- NA
  nb_contacting_stayhome<- NA
  contact_per_explorer<- NA
  contact_per_roamer<- NA
  contact_per_stayhome<- NA
  }

# collate all measures and save it in the results folder
# measure report changed for Emily Oct 2017
measuresReport <- cbind(duration = outbreak_dur,
                        ...
                        nb_in_exp_cat = nb_explorer,
                        nb_in_roam_cat = nb_roamer,
                        nb_stayhome = nb_stayhome,
                        nb_contacting_explorer = nb_contacting_explorer,
                        nb_contacting_roamer = nb_contacting_roamer,
                        nb_contacting_stayhome = nb_contacting_stayhome,
                        contact_per_explorer = round(contact_per_explorer,digits = 3),
                        contact_per_roamer =  round(contact_per_roamer, digits = 3),
                        contact_per_stayhome = round(contact_per_stayhome, digits = 3))
