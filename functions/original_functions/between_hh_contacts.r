...
between_hh_contacts <- function() {

if (verbose) cat("between_hh_contacts() : START\n")
...
infecDogs <- which(allDogs$infectious == 1 & allDogs$dead == 0)
    ...
    #  with a truncated distance kernel. However, 1-x% do not folloe the ban and therefor
...
sapply(infecDogs, function (cur_infecDog) {
  ...
  # candidates that could be contacted by the current infecious dog (cur_infecDog):
...
#   })
  ...
  sapply(distances, function (x) {
    ...
    # add up the vector of 0/1 indicating whether the contact happen or not
...
if (verbose) cat("between_hh_contacts() : END\n")
