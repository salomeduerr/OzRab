...
if (verbose) cat("initialize_simulation() : START\n")
...
allDogs <<- cbind(ID= c(1: nrow(all_Dogs)), # ID that equals row number
                 ...
                 furious=            NA   # 0/1 depending on whether or not the dog develops furious rabies after infection
...
if (verbose) cat("initialize_simulation() : END\n")
