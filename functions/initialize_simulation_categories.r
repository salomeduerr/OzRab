#
# name: initialize_simulation_categories
#
# This file
...
#
# For the work of Emily Oct 2017 we need to read in kernel look-up tables for each community and category
# separately.
# 
# called by OzRab()
#

initialize_simulation_categories <- function () {
  
  if (verbose) cat("initialize_simulation_categories() : START\n")
  ...
  fileName <- paste("scenario",scenarioDataPath,"ex_bamaga_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ex_Bamaga_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ro_bamaga_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ro_Bamaga_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"stayhome_bamaga_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    stayhome_Bamaga_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ex_injinoo_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ex_Injinoo_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ro_injinoo_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ro_Injinoo_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"stayhome_injinoo_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    stayhome_Injinoo_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ex_NewMapoon_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ex_NewMapoon_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  
  fileName <- paste("scenario",scenarioDataPath,"ro_newmapoon_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ro_NewMapoon_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"stayhome_newmapoon_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    stayhome_NewMapoon_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  
  fileName <- paste("scenario",scenarioDataPath,"ex_Seisia_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ex_Seisia_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ro_seisia_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ro_Seisia_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"stayhome_seisia_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    stayhome_Seisia_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ex_Umagico_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ex_Umagico_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"ro_umagico_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    ro_Umagico_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  
  fileName <- paste("scenario",scenarioDataPath,"stayhome_umagico_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    stayhome_Umagico_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  ...
  #### for Emily Oct 2017 - need to add a separte column for all_Dogs randomly assigning
  #### all the dogs a roaming category these cane be change to reflect different numbers of dogs
  #### in each category or community if needed 

  categories <- c(rep("Stayhome",times = round(429*0.333)),
                  rep("Roamer",times = round(429*0.333)),
                  rep("Explorer", times = 429-round(429*0.333)-round(429*0.333))) ###change numbers for differen proportions
 
   all_Dogs$category <- sample(categories, nrow(all_Dogs), replace = F)
  ...
  if (verbose) cat("initialize_simulation_categories() : END\n")
