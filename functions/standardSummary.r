...
standardSummary <- function(step){
  switch(step,
       ...
       # this is to save a column of how many contacts were made from each actegory for further analysis
           if (save_categories_contacts_YN == T) {
             contacting_category <- c()
             
             sapply(allDogs$contactDog, function(x){
               ifelse(x == "unknown; index dog" |  is.na(x),
                      contact_cat <- NA, 
                      contact_cat <- allDogs$category[which(allDogs$dog_ID == x)])
               
               contacting_category <<- c(contacting_category,contact_cat)
             })
             
             allDogs$contacting_category <<- contacting_category
             
             write.csv(count(allDogs$contacting_category), paste(path,"contacting_dog_summary.csv", sep = "/"))
