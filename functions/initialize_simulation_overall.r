#
# name: initialize_simulation_overall
#
# This file
...
#
# For the work of Emily Oct 2017 we need to read in kernel look-up tables for the overal kernel
# based on new contact method by Emily Oct 2017
# 
# called by OzRab()
#

initialize_simulation_overall <- function () {
  
  if (verbose) cat("initialize_simulation_overall() : START\n")
  ...
  # read in the kernel look-up tables for overall from the scenarioDataPath folder
  ...
  fileName <- paste("scenario",scenarioDataPath,"Overall_kernel.csv",sep="/")
  cat("Loading ", fileName,"... ",sep="")
  if (file.exists(fileName)) {
    Overall_kernel <<- read.csv(fileName)
    cat("OK\n")
  } else stop("File missing! Quitting")
  ...
  if (verbose) cat("initialize_simulation_overall() : END\n")
