...
while (!terminationStatus$timeToQuit) {
  ...
  # Between household (hh), within community contacts --> based on the distance kernel from Emily Oct 2017
  if (kernel_type == "community") BHHcontacts <- between_hh_contacts_communities()  # community kernels
  if (kernel_type == "overall") BHHcontacts <- between_hh_contacts_overall()  # overall using new contact method
  if (kernel_type == "category") BHHcontacts <- between_hh_contacts_categories()  # category kernels
  # if (kernel_type == "category_withI27" | 
  #     kernel_type == "category_noI27") BHHcontacts <- between_hh_contacts_categories()  # remove this after
  # #I27 testing done
  
  #BHHcontacts <- between_hh_contacts() # orginal from Salome kernel
