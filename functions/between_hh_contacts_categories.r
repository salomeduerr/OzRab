...
#
# Inclusion of different kernels per community and roaming category (stayhome, roamer, explorer)
# using the kernels produced by Emily Hudson, Oct 2017
...
between_hh_contacts_categories <- function() {
  
  if (verbose) cat("between_hh_contacts_categories() : START\n")
  ...
  sapply(infecDogs, function (cur_infecDog) {
    ...
    ### add another parameter for the category vector in allDogs for Emily work Oct 2017
    cat_cur_infecDog <- allDogs$category[cur_infecDog]
    ...
    sapply(distances, function (x) {
      ...
      if (x > cur_max_dist) {
        ...
        # update using the community and category specific kernels produced by Emily Oct 2017:
        ...
        if (comm_cur_infecDog == "Bamaga" & cat_cur_infecDog == "Stayhome") kernel_table <- stayhome_Bamaga_kernel
        if (comm_cur_infecDog == "Bamaga" & cat_cur_infecDog == "Roamer") kernel_table <- ro_Bamaga_kernel
        if (comm_cur_infecDog == "Bamaga" & cat_cur_infecDog == "Explorer") kernel_table <- ex_Bamaga_kernel
        
        if (comm_cur_infecDog == "Injinoo" & cat_cur_infecDog == "Stayhome") kernel_table <- stayhome_Injinoo_kernel
        if (comm_cur_infecDog == "Injinoo" & cat_cur_infecDog == "Roamer") kernel_table <- ro_Injinoo_kernel
        if (comm_cur_infecDog == "Injinoo" & cat_cur_infecDog == "Explorer") kernel_table <- ex_Injinoo_kernel
        
        if (comm_cur_infecDog == "NewMapoon" & cat_cur_infecDog == "Stayhome") kernel_table <- stayhome_NewMapoon_kernel
        if (comm_cur_infecDog == "NewMapoon" & cat_cur_infecDog == "Roamer") kernel_table <- ro_NewMapoon_kernel
        if (comm_cur_infecDog == "NewMapoon" & cat_cur_infecDog == "Explorer") kernel_table <- ex_NewMapoon_kernel
        
        if (comm_cur_infecDog == "Seisia" & cat_cur_infecDog == "Stayhome") kernel_table <- stayhome_Seisia_kernel
        if (comm_cur_infecDog == "Seisia" & cat_cur_infecDog == "Roamer") kernel_table <- ro_Seisia_kernel
        if (comm_cur_infecDog == "Seisia" & cat_cur_infecDog == "Explorer") kernel_table <- ex_Seisia_kernel
        
        if (comm_cur_infecDog == "Umagico" & cat_cur_infecDog == "Stayhome") kernel_table <- stayhome_Umagico_kernel
        if (comm_cur_infecDog == "Umagico" & cat_cur_infecDog == "Roamer") kernel_table <- ro_Umagico_kernel
        if (comm_cur_infecDog == "Umagico" & cat_cur_infecDog == "Explorer") kernel_table <- ex_Umagico_kernel
  ...
  if (verbose) cat("between_hh_contacts_categories() : END\n")
